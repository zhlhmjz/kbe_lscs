...
class GameObj(KBEngine.Entity):
	...
	def onRemoveBuff(self,buffID):
		buffEntity = self.getEntityByID(buffID)
		if buffEntity == None:
			return
		self.removeBuff(buffEntity)

	def onRemoveSendBuff(self,buffID):
		buffEntity = self.getEntityByID(buffID)
		if buffEntity == None:
			return
	...
	def choose(self,callerID = 0,chooseID = -1):
		DEBUG_MSG("GameObj::choose:[%i].  chooseID:[%s]" % (self.id, chooseID))
		if chooseID == -1:
	...
	def onCauseDamage(self,targetID,damage):
		...
		self.onCauseDamageA(targetID,damage)
	...
	def onKillFollower(self,targetID):
		DEBUG_MSG("GameObj:[%s] onKillFollower:targetID:[%s]" % (self.id, targetID))
		pass
	...
	def sendChoose(self,stringLs):
		...
		self.allClients.choose(stringLs)
	...
	def delSendEnvBuff(self):
		for buff in self.sendBuffList:
			if buff != None and not buff.isDestroyed and buff.envir == 1:
				buff.remove()
