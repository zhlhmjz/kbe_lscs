...
class Avatar(GameObj):
	...
	def init(self):
		GameObj.initAvatar(self,self.cardID,'HERO',self.battlefiled)
		self.avatar = self
	...
	def reqCardList(self,battlefiled):
		...
		self.getFollowerAndHeroList(True)
	...
	def newRound(self,isSelf):
		...
		GameObj.onRoundStart(self,isSelf)
	...
	def endRound(self):
		...
		GameObj.onRoundEnd(self,isSelf)
	...
	def chooseCardWithCardID(self,cardIDList,recvEntity = None,mode = 1):
		...
		for cardID in cardIDList:
			if int(cardID) in d_card_dis.datas:
				...
				e = KBEngine.createEntity('card_' + str(cardID), self.spaceID, tuple(self.position), tuple(self.direction),params)
	...
	def replaceEntity(self,entityID,targetCardID):
		...
		e = KBEngine.createEntity('card_' + str(targetCardID), self.spaceID, tuple(self.position), tuple(self.direction),params)
	...
	def getWeapon(self,weaponID = 0):
		...
		e = KBEngine.createEntity('card_' + str(weaponID), self.spaceID, tuple(self.position), tuple(self.direction),params)
		self.cardEntityList.append(e)
		e.changePos("WEAPON")
	...
	def onTimer(self, tid, userArg):
		...
		if userArg == 1:
			...
			#DEBUG_MSG("Avatar[%i].cell.Client:%s" % (self.id, self.client))
			pass
		elif 4 == userArg:
